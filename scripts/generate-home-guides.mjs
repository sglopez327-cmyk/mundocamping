...
function renderGuideCard(guide) {
  const titulo = escapeHtml(guide.titulo);
  const descripcion = escapeHtml(guide.descripcion);
  const imagen = escapeHtml(guide.imagen);
  const url = escapeHtml(guide.url);
  const alt = escapeHtml(guide.titulo);
  const fallback = guide.imagenFallback ? escapeHtml(guide.imagenFallback) : '';
  const imgTag =
    `<img src="${imagen}" alt="${alt}" width="400" height="300" loading="lazy" decoding="async"` +
    (fallback
  ...
  return (
    `<a href="${url}" class="guide-featured-card">` +
    `<div class="guide-featured-card__media">${imgTag}</div>` +
    `<div class="guide-featured-card__body">` +
    `<h2 class="guide-featured-card__title">${titulo}</h2>` +
    `<p class="guide-featured-card__desc">${descripcion}</p>` +
    `<span class="guide-featured-card__cta cta-button-highlight">Leer guía` +
...
const cardsHtml = GUIDES_DATA.map(renderGuideCard).join('\n                ');
let html = fs.readFileSync(indexPath, 'utf8');

const stackRe = /(<div\s+id="guides-featured-stack"[\s\S]*?>)([\s\S]*?)(<noscript>)/;
if (!stackRe.test(html)) {
  console.error('guides-featured-stack not found in index.html');
  process.exit(1);
}

html = html.replace(stackRe, `$1\n                ${cardsHtml}\n                $3`);
fs.writeFileSync(indexPath, html, 'utf8');
console.log('Injected', GUIDES_DATA.length, 'static guide cards into index.html');
